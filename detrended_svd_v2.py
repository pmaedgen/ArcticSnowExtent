...
method = 'detrended_svd' # options are detrended/nondetrended_covar/svd
...
def monthly_computation_handler(month, lat, lon):
    ...
    ## DETRENDING
    snow_shape = snow_comb.shape
    snow_trend = np.zeros(snow_shape[0]*snow_shape[1]).reshape(snow_shape[0], snow_shape[1])

    for i in range(snow_shape[0]):
        snow_trend[i] = LinearRegression().fit(x_time, snow_comb[i]).predict(x_time)
    ...
    # get eigenvalues of covariance vatrix from singular vals
    vals = np.square(s)
    ...
    for vec in range(plt_rng):
        # Scale with singular values
        nvec = reinsert_rows(vt[vec] * s[vec], suffix=str(month))

        # reshape
        nvec = nvec.reshape(720, 720)
